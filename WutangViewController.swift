...
//  projectchess
//
//  Created by Jan Tlaga on 21/06/2022.
//
...
class WutangViewController: UIViewController {
    ...
    var czas1:Int = 60000
    var czas2:Int = 60000
    @IBOutlet weak var zegar1: UILabel!
    @IBOutlet weak var zegar2: UILabel!
    var timer1:Timer!
    var timer2:Timer!
    let zasady = reguly()
    ...
    func zagrajdzwiek() {
        guard let path = Bundle.main.path(forResource: "intro", ofType:"wav") else {
     ...
     let url = URL(fileURLWithPath: path)
        ...
        do {
            WutangViewController.player = try AVAudioPlayer(contentsOf: url)
            WutangViewController.player.play()
    ...
    func tworzenieszachownicy()
    {
        ...
        zdjecie.tag = -1
        let logo = UIImageView(frame: CGRect(x: 0, y: 0, width: rozmiiarszachownicy, height: rozmiiarszachownicy))
        //logo.frame.origin = zdjecie.frame.origin
        logo.image = UIImage(named: "LOGO")
        logo.layer.opacity = 0.5
        logo.layer.isOpaque = false
        
        view.addSubview(zdjecie)
        view.viewWithTag(-1)!.addSubview(logo)
    }
    ...
    func funtworzenieprzyciskow ()
    {
        for i in 65...128
        {
            ...
            pole.tag = i
            pole.tintColor = UIColor.systemYellow
    ...
    func rozkmina(l1: Int, l2: Int, mat: Bool)
    {
        ...
        if kod != 0 && mozeniemoze == true && (zapisgry.count/64)%2 != 0
        {
            ...
            if kod > 8 && kod < 17 && (l1-1)/8+1 == 5 && removefigura==0
            {
                var x1 = l1
                var x2 = l2
                while x1 > 8
                ...
                if x1 - x2 == 1
                {
                    view.viewWithTag(zapisgry[zapisgry.count-129+l1-1])?.isHidden = true
                    zapisgry[zapisgry.count-65+l1-1] = 0
                }
                if x2 - x1 == 1
                {
            ...
            if zasady.czyatakowane(pole: czarnykrol, kolor: "C",zapisgry: zapisgry,ustawieniefigur: ustawieniefigur) == false
            {
                ...
                zaznaczenie = (zapisgry.count-64)/64
                    timer1 = Timer.scheduledTimer(withTimeInterval: 0.001, repeats: true) { [self]
                        timer1 in
                        self.zegar1.text = "\(self.czas1/1000)"
                        czas1 = czas1-1
                        if czas1 == 0
                        {
                            timer1.invalidate()
        ...
        else if kod != 0 && mozeniemoze == true && (zapisgry.count/64)%2 == 0
        {
            ...
            if kod > 48 && kod < 57 && (l1-1)/8+1 == 4 && removefigura==0
            {
                print("test")
                var x1 = l1
                var x2 = l2
            ...
            if zasady.czyatakowane(pole: bialykrol, kolor: "B",zapisgry: zapisgry,ustawieniefigur: ustawieniefigur) == false
            {
                ...
                view.viewWithTag(kod)?.frame.origin = zamianapole(pole: l2)
                zaznaczenie = (zapisgry.count-64)/64
                    timer2 = Timer.scheduledTimer(withTimeInterval: 0.001, repeats: true) { [self]
                        timer2 in
                        self.zegar2.text = "\(self.czas2/1000)"
                        czas2 = czas2-1
                        if czas2 == 0
                        {
                            timer2.invalidate()
