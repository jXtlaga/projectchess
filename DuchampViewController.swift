...
import UIKit
class DuchampViewController: UIViewController {
    ...
    func mieszanie()
    {
        var zliczanie : [String: Int] = [:]
        for int in 1...8
        {
            if int != 5
            {
                var figura: String!
                let losowa = Int.random(in: 0..<4)
                switch losowa
                {
                case 0:
                    figura = "W"
                ...
                if zliczanie[figura] == nil
                {
                    zliczanie[figura] = 1
                }
                else {zliczanie[figura] = zliczanie[figura]!+1}
                ustawieniepoczatkowe[int]="C"+figura+"\(zliczanie[figura] ?? 0)"
                ustawieniepoczatkowe[56+int] = figura+"\(zliczanie[figura] ?? 0)"
    ...
    func tworzenieszachownicy()
    {
        ...
        zdjecie.frame.origin = CGPoint(x: (self.view.frame.size.width-rozmiiarszachownicy)/2, y: self.view.frame.height/2-150)
